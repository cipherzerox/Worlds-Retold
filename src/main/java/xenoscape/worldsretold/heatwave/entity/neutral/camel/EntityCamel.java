...
public class EntityCamel extends AbstractChestHorse implements IRangedAttackMob, IDesertCreature {
	...
	public boolean getCanSpawnHere() {
		...
		return this.world.provider.getDimension() == 0 && this.world.getBlockState(blockpos.down()).getBlock() == Blocks.SAND && this.world.getLight(blockpos) > 8 && super.getCanSpawnHere();
	}
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
		...
		if (livingdata instanceof EntityCamel.GroupData) {
			i = ((EntityCamel.GroupData) livingdata).variant;
		} else {
			i = this.rand.nextInt(4);
			livingdata = new EntityCamel.GroupData(i);
		}
	...
	protected void playChestEquipSound() {
		this.playSound(SoundEvents.ENTITY_LLAMA_CHEST, 1.0F,
	...
	public boolean canMateWith(EntityAnimal otherAnimal) {
		return otherAnimal != this && otherAnimal instanceof EntityCamel && this.canMate()
				&& ((EntityCamel) otherAnimal).canMate();
	}

	public EntityCamel createChild(EntityAgeable ageable) {
		EntityCamel entityllama = new EntityCamel(this.world);
		this.setOffspringAttributes(ageable, entityllama);
		EntityCamel entityllama1 = (EntityCamel) ageable;
	...
	public void joinCaravan(EntityCamel caravanHeadIn) {
	...
	static class AIHurtByTarget extends EntityAIHurtByTarget {
		public AIHurtByTarget(EntityCamel llama) {
			super(llama, false);
		}
		...
		public boolean shouldContinueExecuting() {
			if (this.taskOwner instanceof EntityCamel) {
				EntityCamel entityllama = (EntityCamel) this.taskOwner;
