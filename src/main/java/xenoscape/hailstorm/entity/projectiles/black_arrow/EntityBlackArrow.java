package xenoscape.hailstorm.entity.projectiles.black_arrow;
...
public class EntityBlackArrow extends EntityArrow implements IProjectile {
	...
	public EntityBlackArrow.PickupStatus pickupStatus;
	...
	public EntityBlackArrow(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
		this.pickupStatus = EntityBlackArrow.PickupStatus.DISALLOWED;
		this.damage = 4.0D;
		this.setSize(0.5F, 0.5F);
	}

	public EntityBlackArrow(World worldIn, double x, double y, double z) {
		this(worldIn);
		this.setPosition(x, y, z);
	}

	public EntityBlackArrow(World worldIn, EntityLivingBase shooter) {
		this(worldIn, shooter.posX, shooter.posY + (double) shooter.getEyeHeight() - 0.10000000149011612D,
				shooter.posZ);
		this.shootingEntity = shooter;

		if (shooter instanceof EntityPlayer) {
			this.pickupStatus = EntityBlackArrow.PickupStatus.ALLOWED;
		}
	}
		...
		if (entity != null) {
			...
			if (entity.attackEntityFrom(damagesource, (float) i)) {
				...
				if (!this.world.isRemote && this.motionX * this.motionX + this.motionY * this.motionY
					...
					if (this.pickupStatus == EntityBlackArrow.PickupStatus.ALLOWED) {
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("pickup", 99)) {
			this.pickupStatus = EntityBlackArrow.PickupStatus.getByOrdinal(compound.getByte("pickup"));
		} else if (compound.hasKey("player", 99)) {
			this.pickupStatus = compound.getBoolean("player") ? EntityBlackArrow.PickupStatus.ALLOWED
					: EntityBlackArrow.PickupStatus.DISALLOWED;
		}
	...
	public void onCollideWithPlayer(EntityPlayer entityIn) {
		if (!this.world.isRemote && this.inGround && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == EntityBlackArrow.PickupStatus.ALLOWED
					|| this.pickupStatus == EntityBlackArrow.PickupStatus.CREATIVE_ONLY
	...
	protected ItemStack getArrowStack() {
		return new ItemStack(MItems.BLACK_ARROW);
	}

	public static void registerFixesSpectralArrow(DataFixer fixer) {
		EntityBlackArrow.registerFixesArrow(fixer, "BlackArrow");
	}
}