...
public class BasicItemStatue extends BasicItem {
	...
	public BasicItemStatue(String name) {
		super(name);
		this.name = name;
				...
				if (flag1) {
					...
					if (!list.isEmpty()) {
						...
						if (!worldIn.isRemote) {
							...
							EntityAutomaton entityarmorstand = new EntityAutomaton(worldIn, d0 + 0.5D, d1, d2 + 0.5D);
							float f = (float) MathHelper
									.floor((MathHelper.wrapDegrees(player.rotationYaw - 180.0F) + 22.5F) / 45.0F)
									* 45.0F;
							entityarmorstand.setLocationAndAngles(d0 + 0.5D, d1, d2 + 0.5D, f, 0.0F);
							entityarmorstand.setEquipmentBasedOnDifficulty(worldIn.getDifficultyForLocation(blockpos));
